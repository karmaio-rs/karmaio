...
//! char devices). Unlike [`crate::driver::ops::read`], these never touch the
...
impl<B: BoundedIoBufMut> Op<ReadStream<B>> {
    pub(crate) fn read(io_handle: &SharedIoHandle, buf: B) -> io::Result<Op<ReadStream<B>>> {
        let data = ReadStream {
...
impl<B: BoundedIoBufMut> Operable for ReadStream<B> {}

impl<B: BoundedIoBufMut> Submittable for ReadStream<B> {
    #[cfg(target_os = "linux")]
...
impl<B: BoundedIoBufMut> Completable for ReadStream<B> {
...
pub(crate) fn read_stream_sync_raw(fd: usize, buf: &mut [u8]) -> io::Result<usize> {
    let len = buf.len();
    #[cfg(unix)]
    {
