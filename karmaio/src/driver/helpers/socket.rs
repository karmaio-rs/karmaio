...
impl Socket {
    ...
    ///
    /// Uses multishot accept under the hood (Linux 6.12+). The request is
    /// submitted when the stream is first polled, which lets a cancellation
    /// token wrap it first. Dropping the stream cancels a submitted request.
    /// The multishot SQE is **not** re-armed after it terminates; call
    /// [`incoming`](Self::incoming) again to start a new one.
    #[cfg(target_os = "linux")]
