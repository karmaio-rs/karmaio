...
//! sockets, char devices). See [`crate::driver::ops::read_stream`] for why the
//! offset-based [`crate::driver::ops::write`] ops are unsuitable for pipes.
...
pub(crate) struct WriteStream<B> {
...
impl<B: BoundedIoBuf> Op<WriteStream<B>> {
    pub(crate) fn write(io_handle: &SharedIoHandle, buf: B) -> io::Result<Op<WriteStream<B>>> {
        let data = WriteStream {
...
impl<B: BoundedIoBuf> Operable for WriteStream<B> {}

impl<B: BoundedIoBuf> Submittable for WriteStream<B> {
    #[cfg(target_os = "linux")]
...
impl<B: BoundedIoBuf> Completable for WriteStream<B> {
